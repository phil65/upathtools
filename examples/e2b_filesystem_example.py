...
async def main():
    ...
    # You can pass an API key directly or use E2B_API_KEY environment variable
    ...
    e2b_fs = E2BFS(api_key=api_key, template="code-interpreter-v1")
...
def sync_example():
    ...
    # Create filesystem instance
    e2b_fs = E2BFS(api_key=api_key)

    try:
        # Use synchronous methods (these are wrapped async methods)
        test_file = "/tmp/sync_test.txt"
        content = b"Hello from synchronous interface!"

        # Write file
        e2b_fs.pipe_file(test_file, content)
        print(f"Written file: {test_file}")

        # Read file
        read_content = e2b_fs.cat_file(test_file)
        print(f"Read content: {read_content.decode()}")

        # Check if file exists
        exists = e2b_fs.exists(test_file)
        print(f"File exists: {exists}")

        # Get file size
        size = e2b_fs.size(test_file)
        print(f"File size: {size} bytes")

        # Clean up
        e2b_fs.rm_file(test_file)
        print("File removed")
        ...
        # Note: sync interface doesn't require explicit session management
        pass
