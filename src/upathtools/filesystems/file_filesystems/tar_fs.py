...
from upathtools.filesystems.base.archive import AbstractArchiveFileSystem
...
class TarFileSystem(AbstractArchiveFileSystem):
        ...
        **kwargs,
        ...
        if compression is None:
            ...
            try:
                ...
                logger.warning("Unable to determine file name, not inferring compression: %s", ex)

            if name is not None:
                compression = infer_compression(name)
                logger.info("Inferred compression %s from file name %s", compression, name)
        ...
        out: dict[str, Any] = {}
        for ti in self.tar:
            info = dict(ti.get_info())
            typ = info["type"]
            assert isinstance(typ, bytes)
            info["type"] = typemap.get(typ, "file")
            name = ti.get_info()["name"]
    ...
    def _get_dirs(self):
        ...
        for member in self.tar.getmembers():
            info: dict[str, Any] = dict(member.get_info())
