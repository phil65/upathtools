...
from upathtools.filesystems import NotionFileSystem
...
TEST_CONTENT = "Hello from NotionFileSystem Test!"
...
@pytest.fixture
def fs():
    """Create a NotionFileSystem instance."""
    assert NOTION_TOKEN
    assert PARENT_PAGE_ID
    fs = NotionFileSystem(token=NOTION_TOKEN, parent_page_id=PARENT_PAGE_ID)
    yield fs
...
@pytest.fixture
def test_page(fs: NotionFileSystem):
...
def test_initialization():
    """Test NotionFileSystem initialization."""
    fs = NotionFileSystem(token=NOTION_TOKEN, parent_page_id=PARENT_PAGE_ID)  # type: ignore
    assert fs.protocol == "notion"
    assert fs.parent_page_id == PARENT_PAGE_ID


def test_read_write(fs: NotionFileSystem, test_page: str):
...
def test_binary_mode(fs: NotionFileSystem, test_page: str):
...
def test_file_operations(fs: NotionFileSystem, test_page: str):
...
def test_context_manager(fs: NotionFileSystem, test_page: str):
...
def test_error_handling(fs: NotionFileSystem):
    ...
    with pytest.raises(ValueError, match="Invalid Notion token") as exc_info:
        NotionFileSystem(token="invalid_token", parent_page_id=INVALID_PAGE_ID)
    assert "Invalid Notion token" in str(exc_info.value)


def test_file_closure(fs: NotionFileSystem, test_page: str):
...
def test_large_content(fs: NotionFileSystem):
...
def test_special_characters(fs: NotionFileSystem):
...
def test_delete_operations(fs: NotionFileSystem):
...
def test_nested_delete(fs: NotionFileSystem):
